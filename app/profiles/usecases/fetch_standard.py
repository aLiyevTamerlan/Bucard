...
class FetchStandardProfile:
    ...
    def fetch_profile(self, ctx):
        ctx.profile = self.repo.get_standard(user=ctx.user)
        return Success() if ctx.profile else Failure(reason='profile_not_found')

    def finish(self, ctx):
        return Result(ctx.profile)


FetchStandardProfile.apply.failures([
    'not_validated', 
    'user_not_exists',
    'profile_not_found', 
])
